 ...
 * @LastEditors: 叮咚蛋
 * @LastEditTime: 2021-01-17 10:03:09
...
/** 
  * @brief 位置模式--通过电流模式实现（本杰明源码pid） FIXME:增加最大速度控制
  */

void VESC_position_mode_I(u8 id)
{
	static float i_term_pos = 0;
	static float i_term_rpm = 0;
	static float prv_angle_error = 0;
	static s32 prv_speed_error = 0;
	float p_term;
	float d_term;
	...
	utils_truncate_number_abs(&i_term_pos, 1.0f - fabsf(p_term));
	...
	s32 rpm = output * VESCmotor[id].instrinsic.MAX_RPM;
	PEAK(rpm, VESCmotor[id].limit.posSPlimit); //最大速度限制
	s32 rpm_error = rpm - VESCmotor[id].valReal.speed;
	// Compute parameters
	p_term = rpm_error * VESCmotor[id].s_pid.kp;
	i_term_rpm += rpm_error * (VESCmotor[id].s_pid.ki * dt);
	d_term = (rpm_error - prv_speed_error) * (VESCmotor[id].s_pid.kd / dt);
	// Filter D
	static float d_filter_rpm = 0.0f;
	UTILS_LP_FAST(d_filter_rpm, d_term, VESCmotor[id].s_pid.kd_filter);
	d_term = d_filter_rpm;

	// I-term wind-up protection
	utils_truncate_number(&i_term_rpm, -1.0f, 1.0f);
	prv_speed_error = rpm_error;

	output = p_term + i_term_rpm + d_term;
	utils_truncate_number(&output, -1.0f, 1.0f);
...
/**
 * @author: 叮咚蛋
 * @brief: 位置模式--通过电流模式实现（增量式pid）
 */

void VESC_position_mode_Current(u8 id) //TODO:待测试
{
	...
	if (VESCmotor[id].limit.isPosLimitOn)
		PEAK(VESCmotor->PIDx.SetVal, VESCmotor[id].limit.maxPosition);
	...
	VESCmotor[id].PIDs.SetVal = VESCmotor[id].PIDx.Udlt * VESCmotor[id].PIDx.uKS_Coe;
	VESCmotor[id].PIDs.CurVal = VESCmotor[id].valReal.speed;
	PID_Operation(&VESCmotor[id].PIDs);
	VESCmotor[id].valSet.current += VESCmotor[id].PIDs.Udlt;
	PEAK(VESCmotor[id].valSet.current, VESCmotor->limit.maxCurrentSet); //最大电流限制
	if (ABS(VESCmotor[id].argum.difPosition) < 60)
		VESCmotor[id].status.arrived = true; //到达指定位置
	else
...
void VESC_position_mode_rpm(u8 id)
{
	...
	//VESCmotor[id].PIDx.Udlt=IIR_filter_two(&IIR_flt,VESCmotor[id].PIDx.Udlt);	
